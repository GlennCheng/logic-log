...
├── YYYY-MM-DD.md                ← 當日完整記錄（含 session 分隔標頭）
├── index.md                     ← 全局摘要索引
...
# ── 在當日檔案加 Session 標頭（方案 B）────────────────────
SESSION_HEADER="## Session $CLAUDE_SESSION_ID（$SESSION_START）"
if ! grep -qF "## Session $CLAUDE_SESSION_ID" "$DAILY_FILE" 2>/dev/null; then
    echo -e "\n$SESSION_HEADER\n" >> "$DAILY_FILE"
fi
...
```bash
TEST_DIR=$(mktemp -d)
LOGIC_LOG_DIR="$TEST_DIR" echo '{"session_id": "test"}' | bash hooks-handlers/session-start.sh > /dev/null
ls "$TEST_DIR"
```
...
檔案：`$LOG_DIR/YYYY-MM-DD.md`
操作：append 完整記錄區塊到檔案末尾。
...
從 `index.md` 過濾當前 session ID（從 context 中取得），格式化輸出：
```
...
讀取 `index.md` 找到本 session 第 3 筆的編號，再從 `YYYY-MM-DD.md` 取出完整區塊。

### today：`/llog today`
顯示今日所有記錄摘要。

讀取今日 `YYYY-MM-DD.md`，列出所有記錄的主題與時間。
...
```bash
grep -r "<關鍵字>" "${LOGIC_LOG_DIR:-$HOME/.claude/logic-logs}/" --include="*.md" -l
```

列出命中的檔案與相關段落。

### 全局編號：`/llog #042`
直接開啟第 042 筆記錄的完整內容，不限 session 或日期。

從 `index.md` 找到 #042 的日期，再從對應 `YYYY-MM-DD.md` 取出完整區塊。

### ref：`/llog ref #042`
把 #042 的邏輯注入當前對話 context，讓後續推理可直接引用。

讀取 #042 完整內容，在對話中顯示並說明：「已載入 #042 的邏輯，後續推理可直接引用此分析。」
````
