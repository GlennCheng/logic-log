...
| `LLM` | 推理/分析主要來自 Claude（識別問題、提出方案） |
...
| 欄位 | 必填 | 說明 |
|------|------|------|
| 類型標籤 | ✅ | `[SESSION \| USER]`、`[SESSION \| LLM]`、`[SESSION \| JOINT]`、`[INSIGHT \| USER]` 等 |
| 📌 主題 | ✅ | 一句話描述核心問題 |
| ❓ 問題/動機 | ✅ | 什麼情況觸發了這個分析？ |
| 🧠 直覺與推理 | ✅ | 用 `使用者：` / `LLM：` 標注各自貢獻；USER 記錄保留使用者原話 |
| 🔍 考量維度 | ✅ | 至少 2 個面向 |
| ❌ 排除項目 | 選填 | 有排除方案時才填 |
| ✅ 結論 | ✅ | 最終判斷或選擇 |
...
index.md 的 TYPE 欄格式：`SESSION:USER`、`SESSION:LLM`、`SESSION:JOINT`、`INSIGHT:USER` 等。
